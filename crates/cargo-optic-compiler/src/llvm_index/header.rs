...
pub(super) fn function(reader: &mut Stream<impl BufRead>) -> io::Result<String> {
    // The function name precedes its argument list. Result types and attributes can contain groups.
    // Prefix, prologue, and personality constants occur after the argument list and before the body.
    ...
    loop {
        ...
        match token.bytes.as_slice() {
            b"{" => return Ok(symbol),
            b"prefix" | b"prologue" | b"personality" => typed_constant(reader)?,
            b"!" => {
                let metadata = reader.required_token()?;
...
pub(super) fn alias_target(reader: &mut Stream<impl BufRead>) -> io::Result<Option<String>> {
    ...
    let first = reader.required_token()?;
    ...
    let mut operand = reader.required_token()?;
    ...
    let target = if typed && operand.bytes.starts_with(b"@") {
        ...
        constant(reader, operand)?;
        None
    };

    while let Some(token) = reader.token(true)? {
        if token.bytes != b"," {
            return Err(invalid(
                token.offset,
...
fn typed_constant(reader: &mut Stream<impl BufRead>) -> io::Result<()> {
    let ty = reader.required_token()?;
    consume_group(reader, &ty)?;
    let mut value = reader.required_token()?;

    if ty.bytes == b"ptr" && value.bytes == b"addrspace" {
        reader.expect(b"(")?;
        reader.group(b'(')?;
        value = reader.required_token()?;
    }

    constant(reader, value)
}

fn constant(reader: &mut Stream<impl BufRead>, token: Token) -> io::Result<()> {
    if token.bytes.len() == 1 && matching_close(token.bytes[0]).is_some() {
        return reader.group(token.bytes[0]);
    }
    ...
    if matches!(token.bytes.as_slice(), b"dso_local_equivalent" | b"no_cfi") {
        reader.required_token()?.symbol()?;
        return Ok(());
    }

    if token.bytes == b"c" {
        let string = reader.required_token()?;
    ...
    // Constant expressions use an opcode followed by a parenthesized operand list. GEP can add flags.
...
mod tests {
    ...
    fn function_header_dimensions() {
        for header in [
            ...
            "{i8, ptr} @f(ptr byval({i8, ptr}) %x) { ret {i8, ptr} zeroinitializer }", // Aggregate types.
            "dso_local noundef i32 @f(\n i32 %x,\n ptr %y\n) #0\n section \"{@fake}\" { ret i32 0 }", // Multiline attributes.
    ...
    fn malformed_headers_fail() {
        ...
        for input in [
            ...
            stream.begin_header(0);
            assert_eq!(
                alias_target(&mut stream).unwrap_err().kind(),
                ErrorKind::InvalidData
            );
        }
    }
