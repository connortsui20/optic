...
#[track_caller]
fn show_request(profile: &str) -> BuildRequest {
    BuildRequest::new(
        "show_fixture",
...
/// Requires this profile to produce a new capture in the fixture workspace.
#[track_caller]
fn capture_show_fixture(optic: &Optic, profile: &str) -> CaptureRecord {
    let outcome = optic
        .capture(&show_request(profile), CapturePolicy::Reuse)
        .unwrap();
...
/// Requires the query to select exactly one captured instance.
#[track_caller]
fn find_instance_reference(optic: &Optic, capture: &CaptureId, query: &str) -> InstanceRef {
...
#[track_caller]
fn read_evidence(optic: &Optic, evidence: &EvidenceRange) -> Vec<u8> {
...
fn shows_whole_functions_and_methods() {
    ...
    let capture = capture_show_fixture(&optic, "release");
    ...
    for (query, expected_file, source_file) in [
        ...
        (
            ...
            "method.txt",
            "source_items.rs",
        ), // Check a whole method.
    ] {
        let reference = find_instance_reference(&optic, capture.id(), query);
        ...
        assert_eq!(read_evidence(&optic, &evidence), expected.as_bytes());
...
fn uses_normalized_loaded_source_with_unicode_bom_crlf_and_spaces() {
    ...
    let optic = Optic::open(&workspace).unwrap();
    let capture = capture_show_fixture(&optic, "release");
    let reference = find_instance_reference(
        &optic,
        capture.id(),
...
fn reports_nonlocal_generated_and_outside_package_source() {
    ...
    fs::write(
        ...
        concat!(
            "#[inline(never)]\n",
            "pub fn outside_function(value: u64) -> u64 { ",
            "std::hint::black_box(value.wrapping_add(13)) }\n",
        ),
    )
    .unwrap();
...
fn returns_exact_optimized_bodies_for_both_supported_stages() {
    ...
    for (profile, stage) in [
        ("release", LlvmStage::LocalThinLtoPostPassManager), // Release uses multiple codegen units.
        ("no-lto", LlvmStage::NoLtoOptimized),               // The profile explicitly disables LTO.
    ] {
        let capture = capture_show_fixture(&optic, profile);
        let found = optic
        ...
        for body in &bodies {
            ...
            let llvm = String::from_utf8(read_evidence(&optic, body.evidence())).unwrap();
...
fn reads_stored_source_and_llvm_after_checkout_edits() {
    ...
    let capture = capture_show_fixture(&optic, "release");
    let reference =
        find_instance_reference(&optic, capture.id(), "show_fixture::source_items::ordinary");
    let source = optic.source(&reference).unwrap();
    let SourceEvidence::Available { evidence, .. } = &source else {
        panic!("expected stored source");
    };

    let source_bytes = read_evidence(&optic, evidence);
    ...
    let llvm_bytes = bodies
        .iter()
        .map(|body| read_evidence(&optic, body.evidence()))
        .collect::<Vec<_>>();
    ...
    assert_eq!(read_evidence(&optic, evidence), source_bytes);
...
fn unsupported_llvm_preserves_source_capture_and_warm_reuse() {
    for profile in [
        "incremental", // The profile enables incremental compilation.
        "cross-thin",  // The profile enables cross-crate ThinLTO.
        "fat",         // The profile enables fat LTO.
    ] {
